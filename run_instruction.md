# Running dns-service-discovery

A DNS server and service discovery layer, written in Python. This file covers how to build
...
```bash
python -m beacon.cli --help
```

The subcommands are `drills`, `check`, `summary`.
...
```bash
python -c "from examples.concurrencyday import main; main()"
```

The full list is `concurrencyday`, `consensusday`, `convergenceday`, `discoveryday`, `edgeday`, `electionday`, `firstzone`, `outagenight`, and others.

## Layout

- `beacon/` the package itself
- `beacon/drills/` the verification organ, a set of measured claims about the
